 ...
 */
int do_open(const char *imgfs_filename, const char *open_mode,
            struct imgfs_file *imgfs_file);

/**
 * @brief Do some clean-up for imgFS file handling.
 *
 * @param imgfs_file Structure for header, metadata and file pointer to be
 * freed/closed.
 */
 ...
 * freed/closed.
 */
enum do_list_mode { STDOUT, JSON, NB_DO_LIST_MODES };
 ...
 * @param json A pointer to a string containing the list in JSON format if
 * output_mode is JSON. It will be dynamically allocated by the function.
 * Ignored for other output modes.
 * @return some error code.
 */
int do_list(const struct imgfs_file *imgfs_file, enum do_list_mode output_mode,
            char **json);
 ...
 * @return Some error code. 0 if no error.
 */
int do_insert(const char *image_buffer, size_t image_size, const char *img_id,
              struct imgfs_file *imgfs_file);
 ...
 * @param imgfs_tmp_bkp_path The path to the a (to be created) temporary imgFS
 * backup file
 * @return Some error code. 0 if no error.
 */
